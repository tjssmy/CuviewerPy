...
{
	if ( ioDeviceIn->readBlock((char *)(&toFill), (uint) sizeof(toFill))
		 == /*(int)*/sizeof(toFill) )
	{
		return true;
	}
...
{
	if ( ((int) ioDeviceIn->readBlock( (char*) toFill, (uint) count) ) == (int)count )
	{
		return true;
	}
	else
...
{
	if ( ((unsigned int) ioDeviceIn->readBlock( (char*) toFill,
			      ...
			      const unsigned int charCount, const bool bytesMirrored)
{
  ...
  if ( ((unsigned int) ioDeviceIn->readBlock( (char*) tempData,
